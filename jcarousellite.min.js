 ...
 *
*/
(function(f){function B(x){if(this.id)this.id+=x}f.jCarouselLite={version:"1.7",curr:0};f.fn.jCarouselLite=function(x){var a=f.extend({},f.fn.jCarouselLite.defaults,x);this.each(function(){function y(){return j.slice(d).slice(0,m)}function n(b){if(z)return false;var c=b>d;a.beforeStart&&a.beforeStart.call(this,y(),c);if(a.circular){if(b>d&&b>=m+i){d%=i;b%=i;k.css(s,-d*o)}else if(b<d&&b<0){d+=i;b+=i;k.css(s,-d*o)}d=b+b%1}else{if(b<0)b=0;else if(b>r-t)b=r-t;d=b;a.btnPrev&&a.$btnPrev.toggleClass(a.btnDisabledClass,
